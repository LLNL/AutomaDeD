...
int main(int argc, char *argv[])
{
	...
	/* Process 3 blocks indefinitely because of a bug */ 
	fault(rank); 
	...
	if (rank == 0) 
	{
		...
		MPI_Bcast((void *)buffer, 512, MPI_INT, 0, MPI_COMM_WORLD);
	} 
	else
	{
		MPI_Bcast((void*) buffer, 512, MPI_INT, 0, MPI_COMM_WORLD);
	...
	/* Each task perform some work here.... */
	if (rank == 0)
		cout << "Computing data..." << endl;

	/* Perform global reduction to collect results */
	int local_result=0;
	int global_result=0;
